...
KERN_DATASET_PATH = "test" # files from where we are loading our data
...
MAPPING_PATH = "mapping.json" # this file will store the mapping of musical symbols to integers
...
def load_songs_in_kern(dataset_path, batch_size ):
...
def preprocess_batch(batch, start_index):
    """Processes a batch of songs and saves them individually."""
    for i, song in enumerate(batch):
        if not has_acceptable_durations(song, ACCEPTABLE_DURATIONS):
            continue
        song = transpose(song)
        encoded_song = encode_song(song)
        save_path = os.path.join(SAVE_DIR, str(start_index + i))
        with open(save_path, "w") as fp:
            fp.write(encoded_song)


def preprocess(dataset_path, batch_size):
    """Processes songs in batches."""
    print("Loading and processing songs in batches...")
    os.makedirs(SAVE_DIR, exist_ok=True)
    processed_count = 0
    for batch in load_songs_in_kern(dataset_path, batch_size):
        preprocess_batch(batch, processed_count)
...
def generate_training_sequences(sequence_length):
    ...
    # Save inputs and targets to separate files
...
def main():
    preprocess(KERN_DATASET_PATH, BATCH_SIZE)
